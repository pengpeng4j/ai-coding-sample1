...
public class JsonUtils {
	...
	public static <T> T json2Object(String json, JavaType javaType) {
		return json2Object(DEFAULT_OBJECT_MAPPER, json, javaType);
	}
	 ...
	 * @return
	 */
	public static <T> T json2Object(ObjectMapper objectMapper, String json, JavaType javaType) {
		try {
			if(StringUtils.isNotBlank(json)) {
	...
	/**
	 * 对给定的ObjectMapper应用默认的设置
	 *
	 * @param objectMapper
	 * @return
	 */
	public static ObjectMapper applyDefaultConfig(ObjectMapper objectMapper) {
		//去掉默认的时间戳格式
		objectMapper.configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, false);
		//设置输入时忽略在JSON字符串中存在但Java对象实际没有的属性
		objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
		objectMapper.setDateFormat(new CompatibleDateFormat());
		...
		objectMapper.registerModule(defaultJdk8Module());
		objectMapper.registerModule(defaultJavaTimeModule());
		return objectMapper;
	}

	public static Jdk8Module defaultJdk8Module() {
		return new Jdk8Module();
	}

	public static JavaTimeModule defaultJavaTimeModule() {
		JavaTimeModule javaTimeModule = new JavaTimeModule();
		javaTimeModule.addDeserializer(LocalDateTime.class, new LocalDateTimeDeserializer(DateTimeUtils.COMPATIBLE_DATETIME_FORMATTER));
		javaTimeModule.addSerializer(LocalDateTime.class, new LocalDateTimeSerializer(DateTimeUtils.DEFAULT_DATETIME_FORMATTER));
	...
	public static class CompatibleDateFormat extends StdDateFormat {

		private final Pattern millisTimestampPattern = Pattern.compile("-?\\d+");

		public CompatibleDateFormat() {
			super(DateTimeUtils.DEFAULT_TIME_ZONE, Locale.SIMPLIFIED_CHINESE, true);
		}

		public CompatibleDateFormat(TimeZone tz, Locale loc) {
			super(tz, loc, true);
		}

		/**
		 * Date转字符串,使用标准格式(yyyy-MM-dd HH:mm:ss)
		 */
		@Override
		protected void _format(TimeZone tz, Locale loc, Date date, StringBuffer buffer) {
			LocalDateTime dateTime = LocalDateTime.ofInstant(date.toInstant(), tz.toZoneId());
			buffer.append(DateTimeUtils.format(dateTime, DateTimeUtils.DEFAULT_DATETIME_PATTERN));
		}
		...
		protected Date _parseDate(String dateStr, ParsePosition pos) throws ParseException {
			if(millisTimestampPattern.matcher(dateStr).matches()) {
				return new Date(Long.parseLong(dateStr));
			} else {
				return DateTimeUtils.parse2Date(dateStr);
			}
		}

		@Override
		public StdDateFormat clone() {
			return new CompatibleDateFormat(_timezone, _locale);
		}
